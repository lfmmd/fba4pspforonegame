...
//----------------------------------------------------------------------------------------------------------
// kovshp - ASIC27A type 1 (Knights of Valour Super Heroes Plus)

static unsigned short kovshp_highlatch_to_arm;
static unsigned short kovshp_lowlatch_to_arm;
...
{
	*((unsigned short*)(PGMARMROM + 0x2892)) = 0x0101;
	*((unsigned short*)(PGMARMROM + 0x289e)) = 0x0107;
	*((unsigned short*)(PGMARMROM + 0x28a4)) = 0x0108;
	*((unsigned short*)(PGMARMROM + 0x28a8)) = 0x0101;
	*((unsigned short*)(PGMARMROM + 0x2bf2)) = 0x4810;
	*((unsigned short*)(PGMARMROM + 0x2bf4)) = 0x800e;
	*((unsigned short*)(PGMARMROM + 0x2c92)) = 0x400f;
	*((unsigned short*)(PGMARMROM + 0x2ce0)) = 0x6c1e;
	*((unsigned short*)(PGMARMROM + 0x2ce2)) = 0x0048;

	for (int i = 0x2ce8; i < 0x2e48; i += 8) { // fix z80 data offsets (offset - 0x9e000)
		unsigned short d = ((PGMARMROM[i+4] << 8) + (PGMARMROM[i+7] << 0)) - 0x09e0;
		PGMARMROM[i+4] = d >> 8;
...
static unsigned short __fastcall kovshp_asic27a_read_word(unsigned int address)
{
	...
	{
		case 0:
			return kovshp_lowlatch_to_68k;

		case 2:
			return kovshp_highlatch_to_68k;
	}

	return 0;
}

static void __fastcall kovshp_asic27a_write_word(unsigned int address, unsigned short data)
{
	switch (address & 6)
	{
		case 0:
			kovshp_lowlatch_to_arm = data;
		return;
		...
		case 2:
		{
			...
			// O(1) switch-based command translation (compiles to jump table)
			switch (asic_cmd) {
			...
			kovshp_highlatch_to_arm = asic_cmd ^ (asic_key | (asic_key << 8));
			pgm_arm7_resume();
		}
		return;
...
static void kovshp_asic27a_arm7_write_long(unsigned int address, unsigned int data)
{
	switch (address)
	{
		case 0x40000000:
		{
			...
			kovshp_lowlatch_to_arm = 0;
			kovshp_highlatch_to_arm = 0;
			if ((kovshp_highlatch_to_68k & 0xFF00) != 0) {
				pgm_arm7_suspend();
			}
		}
		return;
...
static unsigned int kovshp_asic27a_arm7_read_long(unsigned int address)
{
	switch (address)
	{
		case 0x40000000:
...
static int kovshp_asic27aScan(int nAction, int *)
{
	struct BurnArea ba;
	...
	if (nAction & ACB_DRIVER_DATA) {
		...
		SCAN_VAR(kovshp_highlatch_to_arm);
		SCAN_VAR(kovshp_lowlatch_to_arm);
...
{
	pPgmScanCallback = kovshp_asic27aScan;
	...
	Arm7MapMemory(PGMARMRAM0,		0x10000000, 0x100003ff, ARM7_RAM);
	Arm7MapMemory(PGMARMRAM2,		0x50000000, 0x500003ff, ARM7_RAM);
	Arm7SetWriteWordHandler(kovshp_asic27a_arm7_write_word);
	Arm7SetWriteLongHandler(kovshp_asic27a_arm7_write_long);
	Arm7SetReadLongHandler(kovshp_asic27a_arm7_read_long);
	Arm7Close();
}
